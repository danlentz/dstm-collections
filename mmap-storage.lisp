  ...
  (:method ((mf mmapped-indexed-data-file))
    (let* ((header        (header mf))
            ...
            (footer       (when has-footer-p
                            (mem-ref footer-ptr (footer-type-for mf)))))
      (prog1 footer
        (when has-footer-p
...
(defun make-footer (mmapped-file-designator root-offset action &key serial previous-offset
  ...
  (let* ((header            (header mmapped-file-designator))
          ...
          (new-footer       (mem-ref new-footer-ptr (footer-type-for mmapped-file-designator))))
    (prog1 (mmptr-offset new-footer-mmptr)
      (setf
