...
;; no external custom macros, pure portable CL. Includes packages ORD,  
;; RBX, SET, and MAP (the X suffix to avoid clashes with  
...
;; SET code and just ahead of the MAP constructs a couple of RB-Trees  
...
#|
(defun make-tree (&optional (tree (set:empty)))
    (if (= (set:height tree) 10)
      (values (cardinal tree) tree)
      (make-tree (set:add (random 16384) tree))))
...
(let ((xt (set:empty))) (dotimes (ix 100) (setf xt (set:add ix xt))) xt)


(let ((xt (set:empty))) (dotimes (ix 100) (setf xt (set:add (- 100 ix) xt))) xt)
|#
;; -------------------------------------------------------------
