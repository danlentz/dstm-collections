...
;;
;; Once "unfolded" it will accumulate in reverse (for backtracking) as
;;
;; (13 NIL
;;   (12 NIL
;;     (11 NIL
  ...
  (alet (enum index head elt tail accum size whole tree)
    ...
    (dlambda
      ...
      (:reset  (&optional (collection whole))
        (setf
          whole  collection
          tree   (dstm:safe-value collection)
      ...
      (:collect (&optional (count 1))
        ...
        (loop
          ...
          :do (progn
                ...
                (funcall this :next))
          :finally (return (values (reverse (first accum)) (reverse (second accum))))))
      (:state  ()
